...
先从 DevWiki link block 读取 `DevWiki project`。如果 link block 没有 project，执行 `zatools devwiki repo info`；该命令无参数时只输出已配置 project 名称 JSON 数组。只有一个 project 时直接使用；多个 project 时列出候选并让用户选择；没有配置时提示先执行 `zatools devwiki repo add <project> ...`。

从 `repo info <project>` 的 `source` 获取文档库或远端 API 信息，从 `code_repos[].path` 获取已绑定代码仓路径。后续 search/read 使用 `--project <project>`，由 CLI 根据统一配置选择本地或远端查询，不要使用独立路径查询命令。
...
```bash
zatools devwiki read workflow <slug> --view card --project <project>
```
...
```bash
zatools devwiki read workflow workflow-ha-brain-split-protection --view card --project <project>
```

不要写成：

```bash
zatools devwiki read workflow workflow-ha-brain-split-protection.md --view card --project <project>
```
...
| 问题类型 | 读取方式 | 说明 |
|---|---|---|
| 代码定位问题，实现机制问题 | `zatools devwiki read workflow <slug> --view core --project <project>` | 获取文件、函数、接口、配置、日志锚点 |
| 深入背景问题 | `zatools devwiki read workflow <slug> --view explain --project <project>` | 只有用户要求背景、设计意图或 core 不足时读取 |
...
`index` 返回 JSON 数组，每个元素只包含 `type`、`description`、`slug`，不包含 `score`：

```json
[{"type":"workflow","description":"HA 脑裂监控与防护实现定位入口","slug":"workflow-ha-brain-split-protection"}]
```

根据 `description`、`type`、`slug` 判断 top candidates；优先处理 top 5，不要一次展开所有候选。查到结果后继续走 card 验证，`read` 的 kind 使用返回的 `type`，页面参数使用返回的 `slug`。

如果 `index` 没有有效结果，再查 `glossary`：

```bash
zatools devwiki search glossary "<关键词1>" "<关键词2>" --project <project>
```

`glossary` 返回 `glossary`、`type`、`description`、`slug`。它只用于补充术语和别名路由，仍必须通过 card 验证后才能进入正文读取。

不要直接 grep `wiki/index.md` 或 `wiki/glossary.md`。如果所有 CLI 结果都无法读取或 card 都不匹配，就提示用户未从文档中获取到有效内容。
