...
从oroject.yaml获取devwiki的文档库根目录。后续的 search 必须修道 --root 指定文档库根目录
...
```bash
zatools devwiki search workflow "防脑裂" "网关" "ha-group" "gateway" --root <真实文档库根目录>
```
...
```bash
zatools devwiki read workflow <slug> --view card --root <真实文档库根目录>
```
...
```bash
zatools devwiki read workflow workflow-ha-brain-split-protection --view card --root <真实文档库根目录>
```

不要写成：

```bash
zatools devwiki read workflow workflow-ha-brain-split-protection.md --view card --root <真实文档库根目录>
```
