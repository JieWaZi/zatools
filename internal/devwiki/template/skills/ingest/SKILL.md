...
| 检查已有 Wiki、结构化搜索、读取关联 topic/workflow 时 | `references/zatools-devwiki.md` |
| 需要直接执行 qmd query/search/update/status 时 | `references/zatools-qmd.md` |
...
3. `devwiki search index/glossary` 命中低置信、噪声过大或无法排序时，按 `references/zatools-devwiki.md` 升档到 `zatools devwiki search <topic|workflow> <query...>`；只有需要直接执行 qmd query/search 时才读取 `references/zatools-qmd.md`。
4. 如果 `zatools devwiki search topic/workflow` / `qmd search` 报错、超时、collection 未注册或 cache 不可写，降级为 `devwiki search index/glossary`，并在 proposal 中说明本轮 qmd 不可用。
...
- 如果更新 `wiki/index.md`、`wiki/glossary.md` 或 `wiki/log.md`，是否已执行 `zatools devwiki check document` 校验格式。
