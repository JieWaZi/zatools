...
7. 页面必须使用 `devwiki:section` 标记，Topic 和 Workflow 都只保留 `card`、`core`、`explain` 三个 view。
...
- 页面是否包含 `devwiki:section` 的 `card`、`core`、`explain`。
- `wiki/glossary.md` 是否只保存全局术语和稳定入口概念。
- 不要把单个进程、文件名、字段名、配置键、CSV 列、动作码、函数名、日志关键字或检索词当作 glossary 术语。

Glossary 应优先沉淀业务能力、系统能力、跨页面主题、稳定领域概念和常用别名。一个术语说明要回答“这个能力/主题解决什么问题、覆盖哪些关键行为、和哪些场景相关”。
