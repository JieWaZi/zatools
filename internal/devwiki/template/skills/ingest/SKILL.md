...
3. 本地 Wiki 命中低置信、噪声过大或无法排序时，再读取 `references/zatools-qmd.md` 并按其中策略使用 `zatools devwiki search <topic|workflow> <query...>`。
4. 如果 `zatools devwiki search` / `qmd search` 报错、超时、collection 未注册或 cache 不可写，降级为本地 Wiki 搜索，并在 proposal 中说明本轮 qmd 不可用。
