...
重点避免 query 继续用旧内容或维护报告回答。检查过期页面、errata/report 是否 active，report 是否缺少 `exclude_from_query: true`，index/glossary 或页面入口链接是否仍指向旧页面或 report，qmd 搜索是否让旧页面/report 排在权威页前。
