...
只有当 Wiki/workflow 候选不足以确认实现入口，或用户明确要求当前代码实现、调用链、日志出处、配置读取点时，才进入本地代码搜索。
...
- 使用了哪个入口锚点
- 核对后的 top-K 候选，按代码文件归并
- 主调用链
- 关键实现节点
- 尚未解决的缺口

如果最终写成文档，`code_refs` 以代码文件 `path` 为粒度写入 workflow 或 troubleshooting；同一文件只能一条。

同一文件内只记录最多 4 个关键入口 symbol，作为后续追踪索引；不要列出文件内所有方法。更细的方法说明写在正文调用链或关键逻辑中，不放进 `code_refs`。
