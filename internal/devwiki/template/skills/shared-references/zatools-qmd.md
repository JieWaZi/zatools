...
本文件同时规定两件事：
1. **检索路由**：如何在结构化入口搜索、`zatools devwiki search`、`zatools qmd query`、raw/code 核对之间选择
...
### 第 1 档：结构化 Index / Glossary 搜索（默认起点）
...
仅当 `devwiki search` 和结构化入口搜索仍不足，且问题本身是**概念 / 设计 / 意图 / 跨页面关系**类时才启用：
