...
func TestStaticAssetsRespectMarkdownPreviewContract(t *testing.T) {
	...
	if !strings.Contains(indexHTML, `<option value="capability" selected>Capability</option>`) {
		t.Fatal("index.html should default the dimension selector to Capability")
	}
	...
	if !strings.Contains(appJS, "let currentFilter = 'capability'") {
		t.Fatal("app.js should default dimension filtering to Capability")
	}
