...
func TestExtractBuiltinSkillsIncludesQueryGuidance(t *testing.T) {
	...
	if !containsAll(content,
		...
		"`zatools devwiki read <topic|workflow|troubleshooting> <slug> --view card`",
		"禁止用 Read 工具直接读 topic/workflow/troubleshooting 文件",
		"### Step 2: 串行定位候选页面",
		"### Step 3: 候选验证（view=card，逐个读）",
		"### Step 4: 按语义深度阅读",
		"index.md 命中后，禁止再跑当前 Step 的任何后续层",
		"workflow core 的代码表格已覆盖入口、核心逻辑、持久化三层",
		"如果 workflow core 的代码表格已包含文件路径、行号和函数名等精确锚点，跳过 code-tracing.md",
		"核对前先读 `references/code-tracing.md`",
		"### Step 7: 按需沉淀答案",
...
func TestExtractBuiltinSkillsIncludesLocalWikiFirstQmdFallbackGuidance(t *testing.T) {
	...
	if !containsAll(content,
		...
		"多 query 结果使用 RRF",
		"`devwiki search` 底层调用 `qmd search`",
		"qmd 失败 fallback",
