...
func TestRootDevwikiSkillsIncludesCodeGuidance(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("ReadFile(code/SKILL.md) error = %v", err)
	}
	content := string(data)
	if !containsAll(content,
		`name: "devwiki-code"`,
...
func TestRootDevwikiSkillsIncludesStructuredIngestGuidance(t *testing.T) {
	...
	if !containsAll(content,
		...
		"TopicTask / WorkflowTask 必须列出 glossary 候选关键术语",
		"先检查 `wiki/glossary.md` 是否已有关键术语或等价别名；不存在才添加",
		"加载 `devwiki-topic`",
		"加载 `devwiki-workflow`",
		"wiki/topics/<slug>.md",
		"wiki/workflows/<slug>.md",
		...
		"zatools devwiki check document",
		"zatools devwiki check graph",
		"是否已执行 `zatools devwiki check document` 校验格式",
	) {
	...
	checks := []struct {
	...
	}{
		{
			...
			wants: []string{
				...
				"Topic frontmatter 必须尽量填写 `module`",
				"不创建独立模块页面",
				"新建 Topic 后必须检查 `wiki/glossary.md`",
				"先确认 glossary 是否已有关键术语或等价别名",
				"校验 index/glossary/log 格式和 `devwiki:section` 分块",
			},
		},
		{
			...
			wants: []string{
				...
				"只列关键文件、关键入口和关键逻辑点",
				"按 `references/knowledge-placement.md` 分配 card/core/explain/raw",
				"新建 Workflow 后必须检查 `wiki/glossary.md`",
				"先确认 glossary 是否已有关键术语或等价别名",
				"校验 index/glossary/log 格式和 `devwiki:section` 分块",
			},
		},
...
func TestRootDevwikiSkillsIncludesQueryGuidance(t *testing.T) {
	...
	if !containsAll(content,
		...
		"Glossary Alignment、Competitor Check、Card Scoring、Evidence Path 和 Confirmation Actions",
		"不是 agent 自行判断通过",
		"medium / low 时必须停止在 card 阶段",
	...
	if containsAny(content,
		...
		"问题语义识别为 `locate_code`",
		"跳过 code-tracing.md，直接用表格锚点进入代码核对",
	) {
...
func TestRootDevwikiSkillsIncludesZatoolsDevwikiSharedGuidance(t *testing.T) {
	...
	if !containsAll(content,
		...
		"`high`：直接继续，简要说明证据路径",
		"`medium`：只确认问题意图和业务范围，不让用户选择文档",
		"`low`：停止事实回答，请用户补充业务锚点",
		"用户可见的候选名称必须优先使用页面 title、glossary 术语或业务中文名",
		"不要写成 `拨测工具 / dial-test`、`Root Hint 监控配置 / root-hint-monitoring` 这类“名称 / slug”格式",
		"`slug` 只用于内部定位、证据路径记录和执行 `devwiki read` 命令",
		"medium / low 必须在读取 core/explain 前停止",
		"我倾向先按“<主候选>”理解，但这个词在当前 Project Brain 里不是稳定术语",
		"用户只确认问题意图和业务范围，不负责选择文档",
		...
		"目录选择规则",
		"去重与权威来源规则",
		"glossary keywords` 逐行返回 glossary 关键词",
		"low | 0 命中；超过 20 条散点命中",
		"唯一 high 且无竞争候选时可说明依据后读取命中页",
		"必要时先用 `glossary keywords` 做术语对齐",
		"升档后找到候选再进入 Evidence Path",
	...
	checks := []struct {
		...
		},
		{
			...
			wants: []string{
				...
				"同义关键词 grep 两次无果",
				"动态路由/未找到静态调用点",
				"Ruby `method_missing`/动态分发、Python `getattr` 路由、Go `reflect` 调用等场景",
			},
			rejects: []string{
				"代码定位表以代码文件 `path` 为粒度",
				"同一文件只能一行",
...
func TestRootDevwikiSkillsIncludesCodeToDocGuidance(t *testing.T) {
	...
	if !containsAll(content,
		...
		"新建 Topic 或 Workflow 后必须先查 `wiki/glossary.md` 是否已有关键术语或等价别名",
		"Topic 的 sources 不写代码文件路径或 `kind: code`",
		"代码、wiki、raw 冲突",
		"zatools devwiki check document",
		"zatools devwiki check graph",
